...
public class BanishingCharm extends Spell {

	public BanishingCharm() {
		super(SpellInfo.spell_Banish);
		setSpellTexture(new SpellTexture(new ResourceLocation(ModInfo.MODID, "textures/spells/spells.png"), 28, 26, 153, 127));
	}
		...
		if (cast == CastType.rightClick) {
			...
			if (iter != null) {
				while (iter.hasNext()) {
					...
					VectorHelper.moveEntityAwayFromPoint(item, Vector3.fromEntityCenter(player), 0.2);
				}
			}
		}
		return true;
